...
impl EventSubManager {
    ...
    ///
    /// **Required OAuth Scopes**: `channel:moderate` or `moderator:read:banned_users`
    ...
    pub async fn subscribe_to_all_events(
        ...
        if success_count == 0 {
            ...
            warnings.push(
                "❌ All EventSub subscriptions failed! Bot will not receive chat messages or events."
                    .to_string(),
            );
            warnings.push("Required OAuth scopes: user:read:chat, user:write:chat".to_string());
            warnings.push("Please re-authorize with proper scopes in Settings.".to_string());
        }
